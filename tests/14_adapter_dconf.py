...
class TestDconfAdapter(unittest.TestCase):
    ...
    def tearDown(self):
        ...
        runtime_dir = os.path.join(self.test_directory, str(self.TEST_UID))
        if os.path.exists(runtime_dir):
            os.chmod(runtime_dir, stat.S_IREAD | stat.S_IWRITE | stat.S_IEXEC)
    ...
    def test_00_generate_config(self):
        ...
        keyfiles_dir = os.path.join(self.cache_path, self.ca.NAMESPACE, "keyfiles")
        ...
        dbfile_path = os.path.join(self.cache_path, self.ca.NAMESPACE, self.ca.DB_FILE)
    ...
    def test_01_deploy(self):
        ...
        deployed_file_path = os.path.join(self.test_directory, deployed_file_name)
        ...
        dconf_user_file_path = os.path.join(
            self.test_directory, "{}".format(self.TEST_UID)
        )
        self.assertTrue(os.path.isfile(deployed_file_path))
        ...
        with open(dconf_user_file_path, "r") as fd:
            data = fd.read()
            fd.close()
        self.assertEqual(data, self.DCONF_USER_FILE_CONTENTS.format(deployed_file_name))
