...
FC_DEFAULT_PROFILE_DATA = json.dumps({
    'priority': FC_DEFAULT_PRIORITY,
    'settings': {}
})
...
class FleetCommanderADProfileRetriever(object):
    ...
    def _read_profile_data(self, ldap_data):
        ...
        if 'nTSecurityDescriptor' in ldap_data:
            ...
            sd = SecurityDescriptorHelper(
                ldap_data['nTSecurityDescriptor'][0], self)
    ...
    def generate_priority_applies(
            self,
            username, groups, hostname,
            priority, global_policy, profile):
    ...
    def process_profile(
        ...
        if data is not None:
            ...
            # Get priority
            priority = str(
                jsondata.get('priority', FC_DEFAULT_PRIORITY)).zfill(5)
            # Generate file name
